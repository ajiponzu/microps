...
static void ether_dump(const uint8_t *frame, size_t flen)
{
  ...
  flockfile(stderr);
  fprintf(stderr, "        src: %s\n", ether_addr_ntop(hdr->src, addr, sizeof(addr))); // バイト列のmacアドレスを文字列に変換
...
int ether_transmit_helper(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst, ether_transmit_func_t callback)
{
  ...
  return callback(dev, frame, flen) == (ssize_t)flen ? 0 : -1; // コールバック呼び出しにより, Ethernetフレームを出力
}
...
int ether_input_helper(struct net_device *dev, ether_input_func_t callback)
{
  ...
  flen = callback(dev, frame, sizeof(frame)); // 引数で渡された関数をコールバックしてEthernetフレームを読み込む
  ...
  return net_input_handler(type, (uint8_t *)(hdr + 1), flen - sizeof(*hdr), dev); // net_input_handler() を呼び出してプロトコルスタックにペイロードを渡す
}
